...
int add_employee(struct dbheader_t *dbhdr, struct employee_t **employees, char *addstring) {
    ...
    struct employee_t *tmp = realloc(*employees, (dbhdr->count + 1) * sizeof(struct employee_t));
    if (tmp == NULL) {
    ...
    *employees = tmp;
    ...
    strncpy((*employees)[dbhdr->count].address, address, sizeof((*employees)[dbhdr->count].address) - 1);
